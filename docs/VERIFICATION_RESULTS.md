...
| ESLint | Pass |
| Vitest domain and WebMCP stub tests | 13 passed |
...
| Live WebMCP discovery and invocation | All four tools discovered; each returned its expected deterministic stub in the in-app browser |
...
- WebMCP is implemented against the feature-detected draft browser API. All four tools are same-origin and restricted to enumerated fictional fixtures. The write-shaped tools persist nothing and none of the tools makes a network request.

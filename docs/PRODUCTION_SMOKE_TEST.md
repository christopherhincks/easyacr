...
1. `curl --fail --silent https://app.easyacr.com/healthz` returns `{ "ok": true }`.
2. In Supabase Auth, set Site URL to `https://app.easyacr.com`, add `https://app.easyacr.com/tools` to Redirect URLs, and configure a production SMTP sender. From `https://easyacr.com/create-account` or `/sign-in`, request a magic link for a test account. Confirm the verified callback reaches `https://app.easyacr.com/tools` and creates an easyACR session.
